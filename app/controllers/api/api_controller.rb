class Api::ApiController < ApplicationController
  ...
  def handle_api_unauth_error(exception)
    render json: { error: exception.message }, status: :unauthorized and return
  end
  def handle_api_badreq_error(exception)
    render json: { error: exception.message }, status: :bad_request and return
  end
  ...
  def current_user
    raise ApiUnauthError.new("token invalid") if @current_user.blank?
    @current_user
  end
  ...
  def set_current_user
    ...
    raise ApiUnauthError.new("no auth header") if auth_header.blank?
